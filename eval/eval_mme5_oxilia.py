...
if os.environ["CODEBASE"] == "vlm2vec":
    ...
    TOKEN = "<image>"
elif os.environ["CODEBASE"] == "mmE5":
    from mmE5.src.arguments import ModelArguments, DataArguments, TrainingArguments
    from mmE5.src.model import MMEBModel
    from mmE5.src.collator import LlamaEvalCollator
    from mmE5.src.vlm_backbone.llava_next.processing_llava_next import LlavaNextProcessor
    TOKEN = "<|image|>"
elif os.environ["CODEBASE"] == "mmembed":
    from vlm2vec.src.arguments import ModelArguments, DataArguments, TrainingArguments
    TOKEN = ""
...
class MME5CandidateDataCollator(SiglipCandidateDataCollator):
    def __init__(self, image_path_prefix, cand_modal="image,text", image_size=None):
...
@dataclass
class MMEmbedCollactor:

    def __call__(self, examples):
        """
        :param examples: qry, qry_image, pos_text, pos_image
        """
        qiddid = [e[2] for e in examples]
        a=[]
        instruct = ""
        for e in examples:
            d = {}
            if len(e[0])>0:
                d['txt']=".".join(e[0].split('.')[1:])
                instruct=e[0].split('.')[0]
            else:
                d['txt']=""
            if e[1] is not None:
                d['img'] = e[1]
...
class iIiasDatasetForMME5(Dataset):
    """Wraps the HuggingFace iLIAS dataset for mmE5/vlm2vec/mmembed evaluation."""
    def __init__(self, hf_split, is_query=True, mode="image"):
        ...
        self.key_to_id = {}
        self.id_to_key = {}
        # Use data.keys() or enumerate without loading decoded data
        for idx in range(len(self.data)):
            # Access raw key without triggering image decoding
    ...
    def __getitem__(self, idx):
        item = self.data[idx]
        # Return numeric ID instead of string key
        numeric_id = idx  # idx is already the numeric ID
...
class IliasCollatorForMME5:
    """Collator that converts iLIAS items to (text, image, id) tuples for mmE5/vlm2vec/mmembed."""
...
def eval_ilias(args):
    ...
    if os.environ.get("CODEBASE") == "vlm2vec":
        from transformers import LlavaNextProcessor
        processor = LlavaNextProcessor.from_pretrained("llava-hf/llava-v1.6-mistral-7b-hf", trust_remote_code=True)
        eval_collator = EvalCollator(data_args=data_args, model_args=model_args, processor=processor)
    elif os.environ.get("CODEBASE") == "mmE5":
        ...
        eval_collator = LlamaEvalCollator(data_args=data_args, model_args=model_args, processor=processor)
    elif os.environ.get("CODEBASE") == "mmembed":
        eval_collator = MMEmbedCollactor()

    if os.environ.get("CODEBASE") in ("mmE5", "vlm2vec"):
    ...
    # Build ground truth mapping: query numeric id -> list of positive db numeric ids
    print("Building ground truth mappings...")
    query_key_to_pos_db_ids = {}
    for q_idx in range(len(query_hf)):
        # Access without triggering full decode if possible
    ...
    query_dataloader = DataLoader(
        query_dataset, batch_size=batch_size, num_workers=training_args.dataloader_num_workers,
        shuffle=False, collate_fn=lambda batch: eval_collator(query_data_collator(batch))
    )
    db_dataloader = DataLoader(
        db_dataset, batch_size=batch_size, num_workers=training_args.dataloader_num_workers,
    ...
    with torch.no_grad():
        for batch in tqdm(query_dataloader, desc="Encoding iLIAS queries"):
            if os.environ.get("CODEBASE") == "mmembed":
                batch, batch_ids = batch
                query_ids.extend(batch_ids)
            else:
                batch_ids = batch.get('qiddid', [])
                query_ids.extend(batch_ids if not isinstance(batch_ids, torch.Tensor) else batch_ids.cpu().numpy().tolist())
                batch = {k: v.to(device) for k, v in batch.items() if k != 'qiddid'}
            with torch.autocast(enabled=True, dtype=torch.bfloat16, device_type="cuda"):
                if os.environ.get("CODEBASE") == "mmembed":
                    emb = model.encode(batch[:-1], is_query=True, instruction=batch[-1], max_length=4096)['hidden_states']
                else:
                    emb = model(qry=batch)["qry_reps"]
                query_features.append(emb)

        for batch in tqdm(db_dataloader, desc="Encoding iLIAS DB"):
            if os.environ.get("CODEBASE") == "mmembed":
                batch, batch_ids = batch
                db_ids.extend(batch_ids)
            else:
                batch_ids = batch.get('qiddid', [])
                db_ids.extend(batch_ids if not isinstance(batch_ids, torch.Tensor) else batch_ids.cpu().numpy().tolist())
                batch = {k: v.to(device) for k, v in batch.items() if k != 'qiddid'}
            with torch.autocast(enabled=True, dtype=torch.bfloat16, device_type="cuda"):
                if os.environ.get("CODEBASE") == "mmembed":
                    emb = model.encode(batch[:-1], max_length=4096)['hidden_states']
                else:
                    emb = model(tgt=batch)["tgt_reps"]
                db_features.append(emb)
...
class OxfordDatasetForMME5(Dataset):
    """Wraps the HuggingFace Oxford/Paris dataset for mmE5/vlm2vec/mmembed evaluation."""
...
def eval_oxford(args):
    """Evaluate Oxford5k / Paris6k with mmE5/vlm2vec/mmembed models."""
    from datasets import load_dataset as hf_load_dataset
    ...
    data_args.image_dir = getattr(args, 'image_path_prefix', '')
    data_args.max_len = 1024
    training_args.per_device_eval_batch_size = getattr(args, 'batch_size', 16)
    training_args.dataloader_num_workers = 8
    ...
    with torch.no_grad():
        for batch in tqdm(query_dataloader, desc="Encoding Oxford queries"):
            if os.environ.get("CODEBASE") == "mmembed":
                batch, batch_ids = batch
            else:
                batch = {k: v.to(device) for k, v in batch.items() if k != 'qiddid'}
        ...
        for batch in tqdm(db_dataloader, desc="Encoding Oxford gallery"):
            if os.environ.get("CODEBASE") == "mmembed":
                batch, batch_ids = batch
            else:
                batch = {k: v.to(device) for k, v in batch.items() if k != 'qiddid'}
...
def eval_mmeb(args):
    ...
    # assert model_backbone == args.model_backbone
    model_args.model_backbone = args.model_backbone
    model_args.normalize = args.normalize
    model_args.pooling = args.pooling
    ...
    if os.environ["CODEBASE"] == "mmE5" or os.environ["CODEBASE"] == "vlm2vec":
        model = MMEBModel.load(model_args)
        model.eval()
        model = model.to(training_args.device, dtype=torch.bfloat16)
    elif os.environ["CODEBASE"] == "mmembed":
    ...
    query_data_collator = MME5QueryDataCollator(image_path_prefix=args.image_path_prefix, cand_modal=args.query_modal )
    cand_data_collator = MME5CandidateDataCollator(image_path_prefix=args.image_path_prefix, cand_modal=args.cand_modal )
    # Create dataloaders
    batch_size = training_args.per_device_eval_batch_size
    query_dataloader = DataLoader(
        query_dataset,
    ...
    with torch.no_grad():
        ...
        for batch in tqdm(query_dataloader, desc="Encoding queries"):
            ...
            with torch.autocast(enabled=True, dtype=torch.bfloat16, device_type="cuda"):
                if os.environ["CODEBASE"] == "mmembed":
                    query_embeddings = model.encode(batch[:-1], is_query=True, instruction=batch[-1], max_length=4096)['hidden_states']
                    query_features.append(query_embeddings)
                else:
                    # Remove ID field and move to device
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Evaluate retrieval performance using MMEB model')
    ...
    parser.add_argument('--model_backbone', type=str, default='mllama', help='Model backbone')
