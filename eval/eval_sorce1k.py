    ...
    if data == 'flickr30k' or data == 'coco':
        ...
        if 'sorce' in data:
            for level in difficulty:
                output += f"{data}: {level} T2I R@1 {metrics[f'[{level}] image_retrieval_recall@1']:.4f} \n"
                output += f"{data}: {level} T2I R@5 {metrics[f'[{level}] image_retrieval_recall@5']:.4f} \n"
                output += f"{data}: {level} T2I R@10 {metrics[f'[{level}] image_retrieval_recall@10']:.4f} \n"
        else:
        ...
        output += f"{data}: I2T R@1 {metrics['text_retrieval_recall@1']:.4f} \n"
        output += f"{data}: I2T R@5 {metrics['text_retrieval_recall@5']:.4f} \n"
        output += f"{data}: I2T R@10 {metrics['text_retrieval_recall@10']:.4f} \n"
...
def _run_model_batch(model, processor, device, messages_list):
    """
    内部辅助：对一个 batch 的 messages_list 一次性处理，生成 embeddings。
    对齐 fgovd：单次调用 vision 预处理与 processor，然后前向。
    """

    # === 与 fgovd 对齐：对整个 messages_list 一次性处理 ===
    ...
    #     id_dict = SimpleNamespace(justfull=[], multi_img_texts=[])
    # else:
    #     if not hasattr(id_dict, 'justfull'):
    #         id_dict.justfull = []
    #     if not hasattr(id_dict, 'multi_img_texts'):
    #         id_dict.multi_img_texts = []
...
def emb_data(model, processor, dataset, device,
    ...
    # 使用 accelerate 对 DataLoader 做分发
    dataloader = accelerator.prepare(dataloader)
    
    embs = []
    bar = tqdm(total=len(dataloader), desc=f"Embedding {emb_type}") if accelerator.is_main_process else None
    ...
    for batch in dataloader:
        ...
        # Gather (分布式，多卡汇总 embedding)
        batch_embs = accelerator.gather(batch_embs)
    ...
    if all_embs.shape[0] > expected:
        all_embs = all_embs[:expected]
    return all_embs
...
def main(
    ...
    for data_item in datasets:
        if 'sorce' in data_item:
            ...
            # img_prompt = "Find an image caption describing the following everyday image. \nSummarize above image and text in one word: "
            # text_prompt = "<sent>\nSummarize above sentence in one word: "
