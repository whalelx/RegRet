...
def fetch_image(ele: Dict[str, Union[str, Image.Image]], image_patch_size: int = 14, box_op: str = "draw") -> Image.Image:
    if "image" in ele:
        image = ele["image"]
    else:
        image = ele["image_url"]
...
def extract_vision_info(conversations: Union[List[Dict[str, Any]], List[List[Dict[str, Any]]]], return_ids=False) -> List[Dict[str, Any]]:
    vision_infos = []
...
def process_vision_info(
    ...
    for vision_info in vision_infos:
        if "image" in vision_info or "image_url" in vision_info:
            image_inputs.append(fetch_image(vision_info, image_patch_size=image_patch_size, box_op=box_op))
    ...
    for i, vision_info in zip(ids, vision_infos):
        if "image" in vision_info or "image_url" in vision_info:
            ...
            if not has_box or box_mode == "none":
                image_inputs.append(fetch_image(vision_info, box_op="none", image_patch_size=image_patch_size))
                ...
                image_inputs.append(fetch_image(vision_info, box_op="crop", image_patch_size=image_patch_size))
                ...
                image_inputs.append(fetch_image(vision_info, box_op="draw", image_patch_size=image_patch_size))
                id_justfull.append(cnt)
                image_num_for_minibatch.append(1)
                cnt += 1
