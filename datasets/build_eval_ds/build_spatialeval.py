...
image_output_dir = '/mnt/tidal-alsh01/usr/liangxun/data/dataset-spatial-reasoning/SpatialEval/image'
output_json_path = './SpatialEval.json'
breakpoint()
ds = load_dataset("/mnt/tidal-alsh01/usr/liangxun/data/dataset-spatial-reasoning/SpatialEval/vqa")
...
for index, row in tqdm(enumerate(ds['test'])):
    ...
    sharegpt_item = {
        "messages":[
            {
                "role": "user",
                "content": "<image>\n" + row["text"] + post_prompt
            },
            {
                "role": "assistant",
                "content": "Now answer the question.\n"+f"{row['oracle_option']}. {row['oracle_answer']}"
            },
        ],
