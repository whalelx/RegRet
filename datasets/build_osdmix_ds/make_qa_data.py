...
def process_msg(conversations, image_np, mask_np, bbox, image_name, save_dir, image_info):
    ...
    for idx in range(conv_count):
        ...
        if True: # not os.path.exists(save_image_path):
...
def run(chunk_name):
    data_path = "/mnt/tidal-alsh01/usr/liangxun/data/spatialrgpt/spatialRGPT_split2/" + chunk_name + ".json"
    image_folder = "/data/openimagesv7/train/"
...
if __name__ == "__main__":
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=max_workers) as executor:
        futures = [executor.submit(run, chunk) for chunk in chunk_list]
        for future in concurrent.futures.as_completed(futures):
            try:
                future.result()  # Raise any exceptions that occurred in the worker process
...
'''
make qa datas
generate images with box; json for each conversation
'''